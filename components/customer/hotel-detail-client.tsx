...
export function HotelGallery({ images, name }: { images: string[]; name: string }) {
  ...
  return (
    <div className="relative group aspect-[16/9] sm:aspect-[2/1] rounded-xl overflow-hidden bg-surface-deep shadow-sm">
      <Image
        src={active}
        alt={name}
        fill
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-3 border-b border-beige/20 pb-3">
        <h2 className="font-display text-lg sm:text-xl text-charcoal font-semibold">{t("hotel.rooms")}</h2>
        <a
          href={`/search?hotel=${hotel.id}`}
          className="text-xs font-bold uppercase tracking-wider text-champagne hover:underline shrink-0"
        >
      ...
      <div className="space-y-4">
        {standardRooms.map((room) => (
          <RoomCard key={room.id} room={room} tier={tier} onBook={() => openBooking(room)} />
        ))}
        {donorRooms.length > 0 && (
          <>
            <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-champagne/80 pt-4">
...
function RoomCard({
  ...
  return (
    <div className="flex gap-4 rounded-xl border border-beige/35 bg-white/40 p-4 transition-all hover:bg-white/60">
      <div className="relative h-20 w-24 sm:h-24 sm:w-28 shrink-0 rounded-lg overflow-hidden bg-surface-deep shadow-sm">
        <Image
          src={room.images[0]}
          alt={localized.name}
          fill
        ...
        <div>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="text-sm sm:text-base font-bold text-charcoal leading-snug line-clamp-1">
                {localized.name}
              </h3>
              <p className="text-[11px] text-muted/80 font-medium mt-0.5">
                {localized.category} · {localized.bedType}
              </p>
            </div>
            <div className="shrink-0 text-right">
              <p className="text-base font-bold text-champagne leading-none">
                {formatCurrency(room.pricePerNight)}
              </p>
              <span className="text-[9px] font-semibold uppercase tracking-wider text-muted/70 block mt-0.5">
                {t("common.perNight")}
              </span>
            </div>
          </div>
          <p className="mt-2 text-xs text-muted/90 line-clamp-2 leading-relaxed">{localized.description}</p>
        </div>
